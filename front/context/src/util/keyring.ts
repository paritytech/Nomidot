      ...
      .chain(keyringAddress =>
        tryCatch2v(
          () => {
            ...
            throw new Error(
              'The address you are looking for does not exist in keyring'
            );
          },
          e => e as Error
        )
 ...
 * Either<Error,KeyringAddress>.
 */
export function getKeyringAccount(
  keyring: Keyring,
  account?: string
): Either<Error, KeyringAddress> {
 ...
 * Either<Error,KeyringAddress>.
 */
export function getKeyringAddress(
  keyring: Keyring,
  address?: string
): Either<Error, KeyringAddress> {
