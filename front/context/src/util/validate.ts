...
function validateAmount(
  values: SubResults & UserInputs & WithExtrinsic
  ...
  if (amount.isZero()) {
    return left({
      amount: 'Please make sure you are sending more than 0 balance.',
    });
  }

  return right({ amount, ...rest } as SubResults &
    UserInputs &
    WithExtrinsic &
    WithAmount);
}
...
export function validateDerived(
  ...
  const txLength =
    SIGNATURE_SIZE +
    compactToU8a(accountNonce).length +
...
function validateSubResults(
  ...
  return Object.keys(errors).length
    ...
    : right({
        ...
        recipientBalance,
        ...rest,
      } as SubResults & WithExtrinsic & UserInputs);
}
...
function validateUserInputs(
  ...
  return Object.keys(errors).length
    ...
    : right({
        ...
        extrinsic,
        ...rest,
      } as Partial<SubResults> & UserInputs);
}
